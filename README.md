...
- **`video_url` also accepts YouTube links** (in addition to direct file URLs and `gs://` paths), resolved server-side via `yt-dlp` (`is_youtube_url`/`download_youtube_video` in `src/media_engine.py`). `yt-dlp` is told to prefer H.264 over AV1 (Cloud Transcoder's input decoder rejects AV1 — confirmed live via a `MalformattedInput` error) and to try YouTube's Android client before the web client (works around basic bot-detection). **Known limitation**: YouTube's anti-bot enforcement is markedly stronger against cloud/datacenter IPs than residential ones — YouTube URLs work reliably from local dev but can be blocked (`Sign in to confirm you're not a bot`) once this app runs on Cloud Run or similar. The only further fix is cookie-based auth using a real account's exported session, which was deliberately not implemented (personal-account risk, cookie expiry/maintenance burden, firmer ToS exposure than client-spoofing). For production/cloud use, a direct video-file URL or `gs://` path is the reliable option.
